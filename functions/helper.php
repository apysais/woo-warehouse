...
function wwh_important_icon($order_id) {
	$important = WWH_Orders_Meta::get_instance()->important([
		'post_id' => $order_id,
		'action' => 'r',
		'single' => true
	]);
...
function wwh_get_shipping_methods($shipping_obj) {
	$shipping_arr = [];
...
function wwh_shipping_icon($shipping_obj) {
	$ship_method_id = wwh_get_shipping_methods($shipping_obj);
...
function wwh_is_admin() {
	return WWH_User_Check::get_instance()->is_admin();
}

function wwh_is_warehouse() {
	return WWH_User_Check::get_instance()->is_warehouse();
}
...
function wwh_verify_username_password($user, $username, $password) {
	...
	if ( $referrer && !empty($referrer) && !strstr($referrer,'wp-login') && !strstr($referrer,'wp-admin') ) {
